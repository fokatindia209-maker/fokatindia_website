...
import { partnerMenus, roleBasePath } from "./partnerMenuConfig";

export default function Navbar({ role }: { role: "VENDOR" | "SUB_VENDOR" }) {
  ...
  const mainPages = partnerMenus.map((m) => `${basePath}/${m.path}`);
    ...
    <div className="fixed top-0 left-0 right-0 h-16 bg-white border-b shadow-sm flex items-center justify-between px-4 md:px-8 z-50">
      ...
      <div className="flex items-center gap-2 min-w-[220px]">
        {showBackButton ? (
          ...
          >
            <img src={logo} className="h-10 w-auto mt-2" />
